...
__global__ static void kernel_tc(
    ...
    for (Int row = gx; row < numRows; row += BLOCK_DIM_X * gridDim.x) {
        ...
        const Int tailStart = rowStarts[row];
        const Int tailEnd = rowStarts[row+1];
        ...
        for (Int tailOff = tailStart; tailOff < tailEnd; ++tailOff) {
            ...
            if (!isLocalNonZero || isLocalNonZero[tailOff]) {
                ...
                // edges from the head
                Int headEdge = rowStarts[head];
                const Int headEdgeEnd = rowStarts[head + 1];
...
size_t CSRTC::count() {
        ...
        kernel_tc<<<dimGrid_, dimBlock>>>(triangleCounts_, rowStarts_d_, nonZeros_d_, isLocalNonZero_d_, numRows);
