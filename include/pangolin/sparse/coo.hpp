...
/*! \brief a read-only view of a COO suitable for passing to a GPU kernel by value.
...
For read-only GPU access, use the view() method to get a lightweight reference to the data.
*/
template<typename Index>
class COO {
private:
