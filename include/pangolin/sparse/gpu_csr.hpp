...
/*! \brief a read-only view of a GPUCSRm suitable for passing to a GPU kernel by value.
...
Copying to a GPU kernel by value will cause the underling memory to be copied as well.
For read-only GPU access, use the view() method to get a lightweight reference to the CSR data.
*/
template<typename Index>
class GPUCSR {
private:
 ...
 public:
  ...
  PANGOLIN_CUDA_MEMBER uint64_t max_col() const {return maxCol_; } //!< number of matrix columns
 ...
 const Index *deviceRowPtr() { return rowOffset_.data(); }//!< row offset array, valid on device
 const Index *deviceColInd() { return col_.data(); } //!< column index array, valid on device
};
