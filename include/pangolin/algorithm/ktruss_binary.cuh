...
template <size_t BLOCK_DIM_X, typename CsrCooView>
__global__ void core_binary_indirect(UT *keepPointer, UT *gnumdeleted, UT *gnumaffected, bool *globalMtd,bool *assumpAffected,
	const UT k, const size_t edgeStart, const size_t numEdges,
			...
			while(*assumpAffected)
			{
				*assumpAffected = false;
				...
				core_binary_indirect<dimBlock><<<dimGridEdges,dimBlock,0,stream_>>>(destKP,gnumdeleted, 
					gnumaffected,globalMtd,assumpAffected,k, edgeOffset, *selectedOut,
					mat, keep, affected, reversed, firstTry, 6/*Tunable*/);
				cudaDeviceSynchronize();
