                                                                ...
                                                                bool is_divisible)
{
    ...
    __shared__ uint32_t local_part_sizes[PARTITION];
    __shared__ uint32_t total_except_last_local_part; //count of elements in the first N-1 partitions;
    if(threadIdx.x < PARTITION){
        total_except_last_local_part = 0; 
        local_part_sizes[threadIdx.x] = 0;
    }
    __syncthreads();
    uint64_t local_part_size_lim = blockIdx.x == (gridDim.x-1) ? ceil((num_nodes%blockDim.x)/(1.0*PARTITION)) : ceil(blockDim.x/(1.0*PARTITION));
	...
	{		
        ...
        for(uint64_t j=0; j<degree; j++){
            ...
            for (uint8_t cur_part = 0; cur_part < PARTITION; cur_part++) {
        ...
        for (uint8_t i = 0; i < PARTITION-1; i++){       
            for (uint8_t j = 0; j < PARTITION-i-1; j++){
                if (cost[j] < cost[j+1]){
                    float temp = cost[j];
                    cost[j] = cost[j+1];
                    cost[j+1] = temp;
                    uint8_t temp2 = arg_sort[j];
        ...
        bool assigned = false;
        for (uint8_t i = 0; i < PARTITION; i++){
            old_size = atomicAdd((unsigned int*)&local_part_sizes[arg_sort[i]], (unsigned int) 1);
            if(old_size<local_part_size_lim){
        ...
        if(!assigned){
            atomicAdd((unsigned int*)&local_part_sizes[PARTITION-1], (unsigned int) 1);
            new_P[cur_node] = PARTITION-1;
        }
    }
    __syncthreads();
    if(threadIdx.x < PARTITION-1){
        if(local_part_sizes[threadIdx.x] > local_part_size_lim)
            local_part_sizes[threadIdx.x] = local_part_size_lim;
        
        atomicAdd((unsigned int*)&total_except_last_local_part, (unsigned int) local_part_sizes[threadIdx.x]);
        atomicAdd((unsigned int*)&new_cardi[threadIdx.x], (unsigned int) local_part_sizes[threadIdx.x]);
    }
    __syncthreads();
    if(threadIdx.x == 0){
        uint64_t num_for_block = blockIdx.x == (gridDim.x-1) > num_nodes%blockDim.x : blockDim.x;
        atomicAdd((unsigned int*)&new_cardi[PARTITION-1], (unsigned int) num_for_block-total_except_last_local_part);
    }
...
void CrossDecomp<pangolinCOO, PARTITION>::Host_repartition_async(bool one_iter, int num_iter, pangolinCOO COO)
{
    ...
    if(one_iter){
        repartition_kernel<PARTITION><<<dimGrid, dimBlock>>>(  col_P.data(), row_P.data(), 
        ...
        for(int i=0; i<num_iter; i++){
            repartition_kernel<PARTITION><<<dimGrid, dimBlock>>>(  row_P.data(), col_P.data(), 
...
*/
template<typename pangolinCOO, uint8_t PARTITION>
void CrossDecomp<pangolinCOO, PARTITION>::reset_cardi()
{
...
void CrossDecomp<pangolinCOO, PARTITION>::Host_evalEdges_async(pangolinCOO COO)
{
    ...
    evalEdges<PARTITION><<<dimGrid0, dimBlock0>>>(row_P.data(), COO.rowInd_.data(), COO.colInd_.data(), edges_per_part.data(), COO.nnz());
    CUDA_RUNTIME(cudaGetLastError());
    return;
}
...
*/
template<typename pangolinCOO, uint8_t PARTITION>
void CrossDecomp<pangolinCOO, PARTITION>::printEdgesPerPar()
{
    using namespace std;
    cout<<"********************************************************"<<endl;
...
*/
template<typename pangolinCOO, uint8_t PARTITION>
void CrossDecomp<pangolinCOO, PARTITION>::rename()
{
