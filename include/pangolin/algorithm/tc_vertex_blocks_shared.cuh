...
__shared__ uint32_t sharedBitmaps[8 * 1024 / sizeof(uint32_t)]; 
  ...
  for(int bi = blockIdx.x; bi < adj.num_rows(); bi += gridDim.x) { //have to do sizeof the array which needs to be added
    ...
    if (io_e != io_s && (adj.colInd_[io_e - 1] < 8 * 1024 * 8)) {
          bitmaps = sharedBitmaps;
    } else {
          bitmaps = &globalBitmaps[blockIdx.x * bitmapSz];
    }
           Index blk_bound = (io_e + BLOCK_DIM_X - 1) / BLOCK_DIM_X * BLOCK_DIM_X;
            ...
            if (io_e != io_s) {
                ...
                reset_bitmap(bitmaps, first, second);
            } 
            __syncthreads();
  } 
...
#define CASE(const_dimBlock)                                                                                           \
  case const_dimBlock: {                                                                                               \
    ...
    CUDA_RUNTIME(cudaOccupancyMaxActiveBlocksPerMultiprocessor(                                                        \
        &maxActiveBlocks, dyn_blocks_low_tri_kernel<const_dimBlock, Csr>, const_dimBlock, 0));                                 \
